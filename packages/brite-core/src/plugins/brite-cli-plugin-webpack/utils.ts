    ...
    new Promise<IBriteCommandResult>((resolve, reject) => {
        ...
        try {
            ...
            server.listen(port, host, (err) => {
                ...
                logger.log(`[${workspaceName}] webpack-dev-server running on internally at ${host}:${port}`);
                logger.log(`[${workspaceName}] Available locally at https://${publicHost}`);
                logger.log(`[${workspaceName}] App bundles served at https://${publicHost}${publicPath}`);
            });
        } catch (e) {
            result = {
                error: e,
